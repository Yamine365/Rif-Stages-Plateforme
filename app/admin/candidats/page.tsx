...
export default function AdminCandidatsPage() {
  const [candidatures, setCandidatures] = useState<CandidatureExterne[]>(
    mockCandidaturesExternes.filter(c => c.status === 'acceptee' || c.status === 'compte_cree')
  )
  ...
  const filteredCandidatures = candidatures
    .filter(c => {
      if (statusFilter === 'acceptee' && c.status !== 'acceptee') return false
      if (statusFilter === 'compte_cree' && c.status !== 'compte_cree') return false
